...
import { Bot, ShieldAlert, Smartphone, Workflow } from "lucide-react";

import type {
	DashboardData,
	DashboardDesktopStatus,
	DashboardProgram,
...
const defaultDesktopStatus: DashboardDesktopStatus = {
	title: "Desktop Agent Disconnected",
	subtitle:
		"Login desktop app untuk mengirim heartbeat monitoring ke dashboard.",
	isMonitoring: false,
	watchedRulesCount: 0,
...
export default function useDashboard(): DashboardData {
	const [desktopStatus, setDesktopStatus] =
		useState<DashboardDesktopStatus>(defaultDesktopStatus);
	...
	useEffect(() => {
		...
		async function loadDesktopStatus() {
			try {
				...
				const nextDesktopStatus: DashboardDesktopStatus = {
					title: data.status.isMonitoring
						? "Desktop Agent Monitoring Active"
						: "Desktop Agent Connected",
					subtitle: data.status.lastSeenAt
						? `Last heartbeat: ${new Date(data.status.lastSeenAt).toLocaleString()}`
						: "Desktop agent connected.",
					isMonitoring: Boolean(data.status.isMonitoring),
					watchedRulesCount: Number(data.status.watchedRulesCount ?? 0),
					activeWindowTitle: data.status.activeWindow?.title ?? null,
				};

				setDesktopStatus(nextDesktopStatus);
			} catch {
				if (!cancelled) {
					setDesktopStatus(defaultDesktopStatus);
				}
			}
		}
	...
	const statCards: DashboardStatCard[] = [
		{
			...
			value: String(desktopStatus.watchedRulesCount),
			delta: desktopStatus.isMonitoring ? "LIVE" : "IDLE",
			icon: ShieldAlert,
		},
		{
			label: "Interventions Sent",
			value: desktopStatus.activeWindowTitle ? "1" : "0",
			delta: desktopStatus.activeWindowTitle ? "ACTIVE TARGET" : "NO TARGET",
			icon: Bot,
		},
	];
	...
	const activeSession = desktopStatus.isMonitoring
		? {
				eyebrow: "ACTIVE MONITORING",
				title:
					desktopStatus.activeWindowTitle ||
					"Desktop tracker is actively monitoring",
				subtitle: `Desktop agent sedang aktif · ${desktopStatus.watchedRulesCount} synced rules`,
				duration: "Agent is keeping you safe",
			}
		: {
				eyebrow: "TRACKER STANDBY",
				title: "Desktop tracker is not actively monitoring",
				subtitle:
					desktopStatus.watchedRulesCount > 0
						? `Desktop agent connected · ${desktopStatus.watchedRulesCount} synced rules siap dipantau`
						: "Belum ada rule aktif dari desktop agent",
				duration: "Connect to stay safe",
			};

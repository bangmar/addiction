...
import { Loader2, Search, ShieldCheck, Sparkles, TimerReset, Trash2 } from "lucide-react";
...
import DeleteHabitModal from "@/src/components/features/habits/delete-habit-modal";
...
export default function HabitsFeature() {
	...
	const createSubmitError = !canCreateHabit
		? "Please sign in first to save a habit to your workspace."
		: habitsNotice?.type === "error"
			? habitsNotice.message
			: null;
	const shouldShowHabitList = !isLoading;
						...
						<div className='flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between'>
							<div>
								...
								<p className='mt-2 max-w-3xl text-sm text-zinc-500 sm:text-base'>
									Atur kategori kebiasaan, tracking mode, budget harian, dan
									target executable/domain agar sinkron ke desktop agent.
								</p>
							</div>
									...
									<div className='flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between'>
										<div>
											...
											<p className='mt-2 max-w-2xl text-base text-zinc-500 sm:text-lg'>
												Gunakan category prompt untuk generate daftar situs dan
												executable, lalu tetapkan mode moderate atau abstinence
												per habit.
											</p>
										</div>
										...
										<div className='flex flex-wrap gap-3'>
											...
											<AddHabitModal
												onSubmit={canCreateHabit ? handleCreateHabit : undefined}
												isSubmitting={isCreatingHabit}
												submitError={createSubmitError}
												triggerLabel={canCreateHabit ? 'New Habit' : 'Sign in to create'}
											/>
										</div>
									</div>
							...
							<motion.div {...fadeInUp}>
								<SectionCard
									...
									{!isLoading && habits.length === 0 ? (
										<div className='rounded-[24px] border border-dashed border-zinc-200 p-5 text-sm text-zinc-500'>
											Belum ada habit. Buat habit pertama untuk mulai sinkronisasi ke desktop agent.
										</div>
									) : null}
									...
									{shouldShowHabitList
										? habits.map((habit) => {
													...
													<div
														...
														<div className='flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between'>
															<div className='flex gap-4'>
																<IconTile
																	icon={habit.icon}
																	className={cn('border-none', habit.accentClassName)}
																/>
																<div>
																	<div className='flex flex-wrap items-center gap-2'>
																		...
																		<span className={cn('rounded-full px-3 py-1 text-xs font-semibold', modeBadgeClassName)}>
																			{habit.mode}
																		</span>
																	</div>
															...
															<div className='flex flex-wrap gap-3'>
																<AddHabitModal
																	...
																	title={habit.name}
																	description='Saat lanjut dari Step 1, AI akan generate target baru lalu digabungkan dengan target habit yang sudah tersimpan.'
																	submitLabel='Close'
											...
											<Button
												variant='outline'
												size='icon'
												className='size-10 rounded-2xl border-rose-200 bg-rose-50 text-rose-700 hover:border-rose-300 hover:bg-rose-100 hover:text-rose-800'
												onClick={() => openDeleteHabitModal(habit.id)}
												...
												{deletingHabitId === habit.id ? (
													...
													<Trash2 className='size-4' />
														...
														<div className='mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4'>
															<div><p className='text-sm text-zinc-500'>Today</p><p className='mt-1 font-semibold text-zinc-950'>{habit.progressLabel}</p></div>
															<div><p className='text-sm text-zinc-500'>Remaining</p><p className='mt-1 font-semibold text-zinc-950'>{habit.remainingLabel}</p></div>
															<div><p className='text-sm text-zinc-500'>Schedule</p><p className='mt-1 font-semibold text-zinc-950'>{habit.schedule}</p></div>
															<div><p className='text-sm text-zinc-500'>Streak</p><p className='mt-1 font-semibold text-zinc-950'>{habit.streak}</p></div>
														</div>

														<div className='mt-5 rounded-full bg-zinc-100 p-1'>
															<div
																className={cn('h-2 rounded-full', habit.mode === 'Abstinence' ? 'bg-emerald-500' : 'bg-violet-500')}
								...
								<SectionCard
									...
									<div className='space-y-4'>
										{targetGroups.map((group) => (
											<div
												key={group.title}
												className='rounded-[24px] bg-zinc-50 p-4'>
								...
								<SectionCard
									...
									{suggestions.map((suggestion) => (
										<div
											...
											className='rounded-[24px] border border-zinc-200 p-4 shadow-sm'>
											<h3 className='font-semibold text-zinc-950'>
												{suggestion.title}
											</h3>
											<p className='mt-1 text-sm text-zinc-500'>
							...
							<motion.div {...fadeInUp}>
								<SectionCard title='Quick actions' contentClassName='space-y-3'>
									{quickActions.map((action) => (
										<div
											key={action.title}
											className='flex items-start gap-3 rounded-[22px] border border-zinc-200 p-4 shadow-sm'>
											<IconTile
												icon={action.icon}
												className={cn("border-none", action.accentClassName)}
											/>
											<div>

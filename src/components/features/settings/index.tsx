						...
						<div className='flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between'>
							<div>
								...
								<p className='mt-2 max-w-3xl text-sm text-zinc-500 sm:text-base'>
									Halaman settings ini fokus ke data profile, reset password,
									preferensi bahasa, dan login log per device.
								</p>
							</div>
						...
						<div className='space-y-5'>
							<motion.div {...fadeInUp}>
								<SectionCard
									...
									description='Kelola nama, email, dan bahasa aplikasi. Profile picture akan dibahas terpisah.'
									...
									<div className='grid gap-5 xl:grid-cols-[220px_minmax(0,1fr)]'>
										<div className='rounded-[24px] border border-white/80 bg-white/85 p-5 shadow-sm'>
											...
											<div className='mt-4 rounded-2xl border border-dashed border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-500'>
												Profile picture belum diaktifkan di menu settings ini.
											</div>
										</div>
											...
											<div className='grid gap-4 md:grid-cols-2'>
												<div className='space-y-2'>
													...
													<p className='text-xs text-zinc-500'>
														Nama ini akan tampil di workspace dan dashboard.
													</p>
												</div>
												...
												<div className='space-y-2'>
													...
													<input
														...
														value={profileEmail}
														disabled
														className='h-12 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 text-sm text-zinc-500 outline-none'
													/>
													<p className='text-xs text-zinc-500'>
														Dipakai untuk login dashboard dan sinkronisasi akun.
													</p>
												</div>
											</div>
								...
								<SectionCard
									...
									action={
										<div className='flex flex-wrap gap-2 rounded-2xl border border-zinc-200 bg-zinc-50 p-1 text-sm text-zinc-500'>
											{loginDeviceFilters.map((filter) => (
												<button
													key={filter}
													type='button'
													onClick={() => setSelectedLoginDeviceFilter(filter)}
													className={cn(
														"rounded-xl px-3 py-2 transition-colors",
														selectedLoginDeviceFilter === filter &&
									...
									{filteredLoginLogs.length === 0 ? (
										<div className='rounded-[24px] border border-dashed border-zinc-200 px-4 py-6 text-sm text-zinc-500'>
											Belum ada login log untuk filter yang dipilih.
										</div>
									) : null}
									...
									{filteredLoginLogs.map((log) => (
										<div
											...
											<div className='flex items-start gap-4'>
												<IconTile
													icon={Smartphone}
													className={cn("border-none", log.accentClassName)}
												/>
												<div>
								...
								<SectionCard
									...
									<div className='rounded-[22px] border border-emerald-200 bg-emerald-50/70 p-4'>
										...
										<p className='mt-2 text-sm text-emerald-800'>
											Nama profile dan preferensi bahasa siap disimpan ke
									...
									<div className='rounded-[22px] border border-zinc-200 bg-zinc-50 p-4'>
										...
										<ul className='mt-3 space-y-2 text-sm text-zinc-600'>
											<li className='flex gap-2'>
												<UserRound className='mt-0.5 size-4 shrink-0 text-lime-600' />
												<span>
													Perbarui nama user sesuai identitas workspace
												</span>
											</li>
											<li className='flex gap-2'>
												<Globe2 className='mt-0.5 size-4 shrink-0 text-lime-600' />
												<span>Pilih bahasa utama untuk dashboard</span>
											</li>
											<li className='flex gap-2'>
